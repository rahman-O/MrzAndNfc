...
class ResultActivity : AppCompatActivity() {
  ...
  /*      findViewById<TextView>(R.id.output_first_name).text = intent.getStringExtra(KEY_FIRST_NAME)
   ...
   fun ResultScreen(){
       ...
       Column(Modifier.fillMaxSize().background(androidx.compose.ui.graphics.Color.White), verticalArrangement = androidx.compose.foundation.layout.Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
           if (intent.hasExtra(KEY_PHOTO)) {
               val bitmap = intent.getParcelableExtra<Bitmap>(KEY_PHOTO)

                   Image(
                       bitmap = bitmap!!.asImageBitmap(), // ✅ تحويل Bitmap إلى ImageBitmap
                       contentDescription = "Photo"
                   )

           }

              Text(text = intent.getStringExtra(KEY_FIRST_NAME)!!)
              Text(text = intent.getStringExtra(KEY_LAST_NAME)!!)
              Text(text = intent.getStringExtra(KEY_GENDER)!!)
              Text(text = intent.getStringExtra(KEY_CHIP_AUTH)!!)
